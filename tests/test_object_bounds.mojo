...
def test_a_subtree_leaves_out_what_is_beside_it() raises:
    ...
    scene.add_mesh(Mesh(GeometryId(0), MaterialId(0), shown))
    lod.add_level(shown)
    scene.add_lod(lod^)
...
def test_every_kind_of_content_adds_its_box() raises:
    ...
    # Every level adds its box, the hidden ones too, as three.js's levels
    # are children.
    var lod = Lod(root)
    var near = scene.add(Object3D())
    var far = Object3D()
    far.set_position(0, 3, 0)
    var far_node = scene.add(far^)
    scene.add_mesh(Mesh(GeometryId(0), MaterialId(0), near))
    scene.add_mesh(Mesh(GeometryId(0), MaterialId(0), far_node))
    lod.add_level(near)
    lod.add_level(far_node, Length(5.0, METER))
    scene.add_lod(lod^)
    scene.update()
    assert_vector(box_from_object(scene, assets, root).max, 1, 4, 0)
    scene.lods.clear()
    scene.meshes.clear()
