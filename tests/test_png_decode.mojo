...
def profiled_then_gamma_png() -> List[UInt8]:
    """`profiled_png` with a gAMA of 1/2.2 after its profile, as libpng
    writes the pair."""
    return [
        UInt8(137),
        UInt8(80),
...
def srgb_then_profiled_png() -> List[UInt8]:
    """`profiled_png` with an sRGB chunk before its profile: the file says
    how to read it, and the profile does not take that back."""
    return [
        UInt8(137),
        UInt8(80),
        UInt8(78),
...
def test_an_srgb_chunk_before_a_profile_is_kept() raises:
    # The file said how to read itself. A profile after that is not a
    # reason to take the statement back.
    var image = decode(srgb_then_profiled_png())
    assert_equal(image.color_space, SRGB)


def test_a_gamma_after_a_profile_does_not_override_it() raises:
    # libpng writes iCCP before gAMA, so a Display-P3 export with a gamma
    # of 1/2.2 used to read back as plain sRGB: the gamma overwrote the
    # profile's refusal to guess. The profile is what the file means.
    var image = decode(profiled_then_gamma_png())
    assert_equal(image.color_space, UNKNOWN_SPACE)
    with assert_raises():
        _ = texture_from(image)
