...
def test_the_eleven_kinds_are_valid_and_a_twelfth_is_not() raises:
    ...
    assert_false(PassKind(11).is_valid())
...
def test_passes_are_added_inserted_and_removed_in_order() raises:
    ...
    with assert_raises():
        composer.add_pass(Pass(PassKind(11)))
    with assert_raises():
        composer.insert_pass(Pass(PassKind(11)), 0)
    assert_equal(composer.pass_count(), 3)
...
def test_a_disabled_pass_is_skipped_and_a_changed_one_is_checked() raises:
    ...
    composer.passes[1].kind = PassKind(11)
    with assert_raises():
        _ = composer.render(renderer, scene, assets, camera)
    with assert_raises():
