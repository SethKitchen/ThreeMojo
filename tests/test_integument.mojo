...
def test_skin_envelope_has_an_interior() raises:
    ...
    assert_true(skin_distance(dims, field.t1) < 0)
    assert_true(skin_occupancy(dims, field.t1) == SOFT_FILL)
    ...
    assert_true(left_field.distance(left_field.t1) < 0)
...
def test_skin_and_hair_mass_are_positive() raises:
    ...
    var thigh = hair_mass(
        HumanoidSpec(Length(6.0, FOOT), MALE), THIGH_HAIR, RIGHT, step
    )
    assert_true(thigh.mass.to(GRAM) > Float32(0))
    var calf = hair_mass(
        HumanoidSpec(Length(5.5, FOOT), FEMALE), CALF_HAIR, LEFT, step
    )
...
def test_skin_and_hair_refuse_bad_inputs() raises:
    ...
    with assert_raises():
        _ = hair_mass_from_dimensions(
            dims, HairPart(2), hair_tissue(), Length(20.0, MILLIMETER)
        )
    var bad_hair = hair_tissue()
    bad_hair.kind = SoftTissueKind(11)
    with assert_raises():
        _ = hair_mass_from_dimensions(
            dims, THIGH_HAIR, bad_hair, Length(20.0, MILLIMETER)
        )
