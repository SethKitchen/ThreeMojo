...
def test_depth_passes_reads_without_claiming() raises:
    # The read half of `test_depth`. Asking twice must give the same answer,
    # because a translucent surface must not take ownership of the pixel.
    var fb = Framebuffer(2, 2, Color(0, 0, 0))
    assert_true(fb.depth_passes(0, 0, 0.5))
    assert_true(fb.depth_passes(0, 0, 0.5))
    assert_equal(fb.depth_at(0, 0), inf[DType.float32]())
    # And it agrees with `test_depth` about what is nearer.
    _ = fb.test_depth(0, 0, 0.5)
    assert_false(fb.depth_passes(0, 0, 0.9))
    assert_true(fb.depth_passes(0, 0, 0.1))


def test_depth_passes_rejects_a_coordinate_outside_the_image() raises:
    var fb = Framebuffer(2, 2, Color(0, 0, 0))
    with assert_raises():
        _ = fb.depth_passes(-1, 0, 0.5)
    with assert_raises():
        _ = fb.depth_passes(0, -1, 0.5)
    with assert_raises():
        _ = fb.depth_passes(2, 0, 0.5)
    with assert_raises():
        _ = fb.depth_passes(0, 2, 0.5)


def test_blending_mixes_by_alpha_in_linear_light() raises:
    # Half of white over black is half the *light*, which displays as 188.
    # Blending the encoded bytes would give 128, which is 21.6% of the light.
    var fb = Framebuffer(2, 2, Color(0, 0, 0))
    fb.blend_pixel(0, 0, FloatColor(1.0, 1.0, 1.0, 0.5))
    assert_equal(fb.get_pixel(0, 0).r, UInt8(188))


def test_blending_with_full_alpha_replaces() raises:
    var fb = Framebuffer(2, 2, Color(10, 20, 30))
    fb.blend_pixel(0, 0, FloatColor(1.0, 1.0, 1.0, 1.0))
    assert_equal(fb.get_pixel(0, 0).r, UInt8(255))


def test_blending_with_no_alpha_leaves_the_pixel_alone() raises:
    var fb = Framebuffer(2, 2, Color(10, 20, 30))
    fb.blend_pixel(0, 0, FloatColor(1.0, 1.0, 1.0, 0.0))
    assert_equal(fb.get_pixel(0, 0).r, UInt8(10))
    assert_equal(fb.get_pixel(0, 0).b, UInt8(30))


def test_blending_clamps_an_alpha_outside_zero_to_one() raises:
    # Nothing in the renderer produces one: opacity is validated and
    # barycentric weights cannot take an interpolated value outside its
    # corners. `blend_pixel` is public, though, and an alpha of two would
    # otherwise subtract what is already there.
    var fb = Framebuffer(2, 2, Color(10, 20, 30))
    fb.blend_pixel(0, 0, FloatColor(1.0, 1.0, 1.0, 2.0))
    assert_equal(fb.get_pixel(0, 0).r, UInt8(255))
    var other = Framebuffer(2, 2, Color(10, 20, 30))
    other.blend_pixel(0, 0, FloatColor(1.0, 1.0, 1.0, -1.0))
    assert_equal(other.get_pixel(0, 0).r, UInt8(10))
