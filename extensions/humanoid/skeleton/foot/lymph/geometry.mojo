...
"""Foot lymphatic meshes from stature and sex.

    var person = HumanoidSpec(Length(6.0, FOOT), MALE)
    var trunk = foot_lymph(person, DORSAL_LYMPHATICS)
...
def foot_lymph(
    ...
    """Return one lymphatic trunk sized for `spec`.

    Args:
        spec: Standing height and osteological sex.
        part: Which trunk to mesh.
    ...
    """
    return lymph_from_dimensions(
        foot_dimensions(spec.stature, spec.sex, side), part, detail
    )


def lymph_from_dimensions(
    dimensions: FootDimensions, part: FootLymph, detail: Int = 8
) raises -> BufferGeometry:
    """Return one lymphatic mesh for already-computed dimensions.
    ...
    var field = display_lymph_field(dimensions, part)
