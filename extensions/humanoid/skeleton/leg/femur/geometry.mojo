...
marching tetrahedra. Each cube of the bounding grid splits into six
tetrahedra. Connectivity comes from the field. It does not come from a
deformed capsule. Normals come from the field's gradient. Texture
coordinates are cylindrical around y: `u` around, `v` up.
...
def femur_from_dimensions(
    ...
    var span_x = field.high.x - field.low.x
    var span_y = field.high.y - field.low.y
    var span_z = field.high.z - field.low.z
    ...
    for iz in range(sz):  # pragma: no branch
        var z = field.low.z + Float32(iz) * dz
        for iy in range(sy):  # pragma: no branch
            var y = field.low.y + Float32(iy) * dy
            for ix in range(sx):  # pragma: no branch
                var x = field.low.x + Float32(ix) * dx
    ...
    for index in range(count):  # pragma: no branch
        ...
        if u < 0:
            u = u + Float32(1)
        var v = _clamp01((py - field.low.y) / span_y)
        uvs.append(u)
        uvs.append(v)
...
def _require_triangles(indices: List[Int]) raises:
    """Refuse an empty isosurface."""
    if len(indices) == 0:
        raise Error("A femur field produced no surface")
