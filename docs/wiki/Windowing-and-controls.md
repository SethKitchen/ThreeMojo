...
three.js drags the outermost `Group` when `transformGroup` is set. This port drags the outermost node of `objects` above the node struck instead. It does not look for a node that `group()` built.

Everything the `Raycaster` tests can be picked, lines, points and sprites included. The raycaster's line and point thresholds apply, as in three.js. See [Raycasting](Raycasting#lines-points-and-sprites). Touch input and the cursor style are not ported.
