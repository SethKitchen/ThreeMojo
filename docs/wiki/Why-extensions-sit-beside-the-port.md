...
`extensions/` is that split. The first path is `humanoid/skeleton/leg/`. A later bone lands next to the femur, tibia, fibula and patella. The knee tissues sit in `leg/knee/`. Grass, terrain and water each get their own folder under `extensions/`.
