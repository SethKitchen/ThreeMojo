...
The core library ports three.js. An extension adds content that three.js does not have: a femur, later a tree, a building, water.
...
`extensions/` is that split. The first path is `humanoid/skeleton/leg/femur/`. A later bone lands next to it. Grass, terrain and water each get their own folder under `extensions/`.
