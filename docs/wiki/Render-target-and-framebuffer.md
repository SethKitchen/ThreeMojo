...
`FloatColor(r, g, b, a)` is four floats in linear light. It is also three.js's `Color`, with an alpha added. The setters that take sRGB decode on the way in. The getters encode on the way out, as three.js does with its default color space.
...
| `FloatColor(hue=h, saturation=s, lightness=l)` | An sRGB color from HSL, decoded. three.js's `setHSL`. The hue wraps. The other two clamp. |
| `encode() -> Color` | Encode linear light to sRGB bytes, clamped. |
| `hex() -> Int` | The encoded color as a 24-bit value. three.js's `getHex`. |
| `hsl() -> HSL` | The encoded color as hue, saturation and lightness. three.js's `getHSL`. |
...
| `lerp_hsl(other, alpha)` | Move toward `other` in HSL. The hue goes the long way, as in three.js. |
| `offset_hsl(h, s, l)` | Add to the hue, saturation and lightness. three.js's `offsetHSL`. |
| `multiply(other)`, `add(other)` | Red, green and blue only. Alpha is kept. |
| `a == b` | Every channel equal. three.js's `equals`. |

`HSL(hue, saturation, lightness)` holds the three floats `hsl()` returns. A gray has a hue and a saturation of zero. CSS color names and strings are not ported.
