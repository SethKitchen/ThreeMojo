...
| `max_scale() -> Float32` | The longest axis: the most any direction is stretched. |
...
An empty box or sphere holds no points. It is a value, not an error. Expanding one by a point gives the bound of that one point.
...
| `apply_matrix4(m)` | Move the center. The radius grows by the largest scale. |
