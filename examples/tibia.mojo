...
"""A gallery of tibias at different proportions, turning under a lamp.

    mojo run -I . examples/tibia.mojo [path.png]

The page is Tibia. Four adults stand in a row: a five-foot female, a
...
comptime DEFAULT_OUTPUT = "out/tibia.png"
comptime WIDTH = 320
comptime HEIGHT = 200
comptime FRAMES = 36
...
def frame_at(
    ...
    Args:
        ...
        node: The parent of every tibia.
...
def add_tibia(
    ...
    """Place one tibia on the gallery row and print its mass.
    ...
    var shape = assets.geometries.add(tibia(person, detail=DETAIL))
    var placed = Object3D()
    placed.set_position(x, 0, 0)
    placed.set_euler(
        Angle(8.0, DEGREE), Angle(0.0, DEGREE), Angle(-12.0, DEGREE)
    )
...
def main() raises:
    ...
    var lamp = Object3D()
    lamp.set_position(0.40, 0.45, 0.85)
    ...
    camera.place(Vector3(0.18, 0.04, 1.15), Vector3(0.0, 0.0, 0.0))
